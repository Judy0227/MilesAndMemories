...
export function handleSignUp() {
   ...
   createUserWithEmailAndPassword(auth, email, password).then((userCredential) => {
    ...
    showMessage('Account created successfully', 'signup-msg')
    ...
    if (errorCode === "auth/email-already-in-use") {
      showMessage("Email Address Already Exists !!", "signup-msg");
    } else {
      showMessage("Unable to create User", "signup-msg");
...
export function handleLogin() {
  ...
  showMessage('Login is Successful', 'login-msg')
const user = userCredential.user;
...
.catch((error) => {
    ...
    if(errorCode === 'auth/invalid-credential') {
        showMessage('Incorrect Email or Password', 'login-msg')
    }
    else {
        showMessage('Account does not exist', 'login-msg')
    }
})
}